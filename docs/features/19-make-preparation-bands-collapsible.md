...
- preparation bands use the lightweight category-like divider treatment, not the card or accordion
  alternatives;
- preparation bands show the preparation plus category name and an entry count;
...
- Each preparation band shows its preparation plus category name and an entry count, using
...
- Preparation bands do not display "prep type subcategory" or any equivalent model explanation in the
  user interface.
- A preparation band displays its preparation plus category name and entry count, for example
  `Raw Fish` and `2 entries`.
