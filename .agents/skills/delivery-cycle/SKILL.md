...
This repository is **local-only**: there is no git remote, no `origin`, and no pull requests. Never
push, fetch, or check a remote. The cycle ends at a local merge into `main`, and "pre-PR" means
"before that merge".
...
1. `git status --short --branch` and `git --no-pager log --oneline -10` - the current branch, whether
   it is `main` or a working branch, and whether there are uncommitted changes.
...
| Plan tasks are complete, targeted validation passes, and `prepare` has not run for these changes | **prepare** | Run the pre-merge review. |
...
| The current branch is not `main`, is committed, and is ahead of `main` | **merge into main** | Fast-forward or merge the branch into `main`. |
| The work is merged into `main` with a clean worktree | **summarise** | Report what shipped and what is next. |
...
an ADR needs an amendment or a new ADR first. After each task run the narrowest relevant check for
what changed, widening to the coverage, lint, and end-to-end scripts when the change warrants them.
Update the feature status to `In progress` when work actually begins.
...
Instruct a subagent to run the `prepare` skill on the branch, telling it explicitly that the
repository has no remote so it reviews the local diff against `main` rather than looking for
`origin`. Do not run it inline in this conversation and do not review the branch yourself in its
place. Report its findings verbatim enough that each one is actionable.
...
Merge the working branch into `main` locally, resolving conflicts with the
...
- Honour the status and dependency rules in `docs/features/README.md`.
