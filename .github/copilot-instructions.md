...
- Maintain explicit coverage and citations. Parse all authored records with Zod during development/CI; missing citations, invalid references, invalid status ownership, and duplicate assessment pairs must fail validation.
...
- Guidance text is manually reviewed and succinctly paraphrased from authoritative sources. Do not scrape, fetch, infer, or automatically update advice. A citation needs a durable HTTPS URL and exact locator.
