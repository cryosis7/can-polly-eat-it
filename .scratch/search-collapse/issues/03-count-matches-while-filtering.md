...
- [ ] While filtering, every rendered category row shows its match count, including roots and open
  rows, and its accessible name reads like "Tea, level 2, 1 match".
- [ ] While filtering, every band shows its match count and its accessible name reads like
  "Smoked Fish, 3 matches".
- [ ] Singular and plural are correct for both rows and bands.
- [ ] While browsing, nothing changes: a collapsed non-root row shows its chip and "N entries"; open
  and root rows show no category count; bands show "N entries"; accessible names are unchanged.
- [ ] A bare scope change is not filtering and keeps "entries" wording.
- [ ] Page tests cover visible text and accessible names for a root, a nested row, and a band, in
  both wordings.
- [ ] Playwright scenarios whose count or accessible-name assertions under a filter now read matches
